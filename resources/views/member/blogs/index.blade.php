<x-app-layout>
    ...
    <x-slot name="headerRight">
        <form action="{{ route('member.blogs.index') }}" method="get">
             <x-text-input id="search" name="search" type="text" class="p-1 m-0 md:w-72 w-80 mt-3 md:mt0"
                            ...
                            <tr>
                                ...
                                <td class="border px-6 py-4">
                                    ...
                                    <div class="block lg:hidden text-sm text-gray-500">
                                        {{ $values->status }} | {{ $values->created_at->isoFormat('dddd, D MMMM Y') }}
                                    </div>
                                </td>
                                <td class="border px-6 py-4 text-center text-gray-500 text-sm hidden lg:table-cell">
                                    {{ $values->created_at->isoFormat('dddd, D MMMM Y') }}</td>
                                <td class="border px-6 py-4 text-center text-sm hidden lg:table-cell">{{ $values->status }}</td>
                                <td class="border px-6 py-4 text-center">
                                    <a href='{{ route('member.blogs.edit', ['post' => $values->id]) }}' class="text-blue-600 hover:text-blue-400 px-2">edit</a>
                                    <a href='' class="text-blue-600 hover:text-blue-400 px-2">lihat</a>

                                    <form class="inline" onsubmit="return confirm('Apa yakin Anda Yakin Untuk Menghapus?')"
                                          method="post" action="{{ route('member.blogs.destroy', ['post' => $values->id]) }}" >
                                        @csrf
                                        @method('DELETE')
